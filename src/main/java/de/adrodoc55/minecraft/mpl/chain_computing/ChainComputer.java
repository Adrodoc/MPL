...
public interface ChainComputer {
    ...
    public default CommandBlockChain computeOptimalChain(CommandChain input) {
        return computeOptimalChain(input, new Coordinate3D(Integer.MAX_VALUE, Integer.MAX_VALUE, Integer.MAX_VALUE));
    }

    public abstract CommandBlockChain computeOptimalChain(CommandChain input, Coordinate3D max);

    public default CommandBlockChain toCommandBlockChain(CommandChain input, List<Coordinate3D> coordinates) {
        List<Command> commands = input.getCommands();
        if (commands.size() >= coordinates.size()) {
            throw new IllegalArgumentException("To generate a CommandBlockChain one additional Coordinate is needed!");
        }
